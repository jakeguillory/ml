...
void perceptron_trick(Model *model, double temp_feature_a,
  ...
  int pred = prediction(model, temp_feature_a, temp_feature_b);
...
int prediction(Model *model, double temp_feature_a, double temp_feature_b) {
  return step(score(model, temp_feature_a, temp_feature_b));
}

double error(Model *model, double temp_feature_a, double temp_feature_b,
             double temp_label) {
  int pred = prediction(model, temp_feature_a, temp_feature_b);
  if (pred == temp_label)
    return 0;
  return fabs(score(model, temp_feature_a, temp_feature_b));
}

double mean_perceptron_error(Model *model, double *features_a,
                             double *features_b, double *labels,
                             int num_features) {
  double total_error = 0.0;
  for (int i = 0; i < num_features; i++)
    total_error += error(model, features_a[i], features_b[i], labels[i]);
  return total_error / num_features;
}
